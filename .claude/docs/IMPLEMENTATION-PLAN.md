...
- [ ] 4.1 核心流程整合測試（預約狀態機、RLS 權限）
- [ ] 4.2 安全檢查：金鑰只在環境變數、RLS 全表覆蓋、LINE webhook 簽章驗證
- [ ] 4.3 Vercel 正式部署 + `.env.example` 與 README 部署文件
- [ ] 4.4 管理員功能：成員邀請/停用
- [ ] 4.5 手機實機檢查（LINE in-app browser 開啟流程）
- [ ] 4.6 分會試用（先 3–5 人小圈測試，再全員推廣）
